...
func (datePicker *DatePicker) callback(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	case cmdDayClick:
		...
		datePicker.onSelect(ctx, b, update.CallbackQuery, time.Date(datePicker.year, datePicker.month, st.param, 0, 0, 0, 0, time.Local))
	case cmdCancel:
		...
		datePicker.onCancel(ctx, b, update.CallbackQuery)
	case cmdPrevYears:
