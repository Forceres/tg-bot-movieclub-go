...
func (kb *Keyboard) callbackAnswer(ctx context.Context, b *bot.Bot, update *models.Update) {
...
func (kb *Keyboard) callback(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if errBtnNum != nil {
		...
		kb.callbackAnswer(ctx, b, update)
		return
	}
	...
	if kb.handlers[btnNum].Handler != nil {
		kb.handlers[btnNum].Handler(ctx, b, update, kb.handlers[btnNum].data)
	}
